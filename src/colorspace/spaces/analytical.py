...
class AnalyticalParams:
    ...
    @classmethod
    def from_dict(cls, d: dict) -> "AnalyticalParams":
        return cls(
            M1=np.array(d["M1"]),
    ...
    @classmethod
    def load(cls, path: str | Path) -> "AnalyticalParams":
        with open(path) as f:
            return cls.from_dict(json.load(f))


def oklab_params() -> AnalyticalParams:
...
class AnalyticalSpace(ColorSpace):
    ...
    def __init__(self, params: AnalyticalParams | None = None, surround: float = 0.5,
                 neutral_correction: bool = False, ab_rotate_deg: float = 0.0):
        if params is not None:
            self.params = params
        elif _PARAMS_PATH.exists():
            self.params = AnalyticalParams.load(_PARAMS_PATH)
        else:
