...
map to $a\!=\!b\!=\!0$ (chroma $<10^{-6}$), and a rigid rotation of
...
(29.20). It also leads on the held-out MacAdam~1974 dataset (19.51
vs 22.13) and on a 3{,}552-judgement Human-Feedback set
(23.26 vs 62.54). A held-out 20\% test split shows a $+1.77$
train$\to$test STRESS gap, yielding a cross-validated point estimate
of $\sim$24.3 that still beats every tested competitor.
...
sRGB/P3/Rec.2020, it wins 66 of 90 vs OKLab. The transform is
...
but is computationally expensive and exhibits substantial achromatic
chroma leakage ($\bar{C}>1$ for D65 grays in our re-evaluation).
...
\begin{enumerate}
  \item \textbf{MetricSpace v21}: a 72-parameter analytical color
    ...
    CIEDE2000. The same space leads on the held-out MacAdam~1974 set
    (19.51 vs 22.13) and on a Human-Feedback judgement set (23.26
    vs 62.54).
  ...
  \item A neutral correction that guarantees $C < 10^{-6}$ for
    achromatic colors, decoupling measurement-optimal training from
    generation-grade neutrality.
  ...
  \item \textbf{GenSpace v0.11.1}: a generation-optimized companion
    ...
    that wins 66 of 90 metrics versus OKLab on a 3{,}038-pair
...
a regularizer ($\lambda_\mathrm{He}\!=\!0.05$) and is therefore not
fully independent; its \stress\ is reported for transparency.
MacAdam~1974 (128~pairs, 10\textdegree) is held out entirely and
never seen during optimization. v21 outperforms CIEDE2000 on
MacAdam (19.51 vs 22.13) — an honestly-held-out result.
...
19.51 (CIEDE2000: 22.13, $-$11.8\%). On a third Human-Feedback
judgement set (3{,}552 pairs collected on monitor under controlled
viewing conditions during development) it scores 23.26 (CIEDE2000:
62.54, $-$62.8\%); we include this last dataset because it is the
only large-scale screen-condition validation set we have access to,
and modern UI applications operate under those conditions.
...
\begin{table}[t]
  \centering
  \caption{\stress\ across three datasets. Lower is better. All baselines use Euclidean distance in their native Cartesian coordinates.}
  \label{tab:stress}
  \begin{tabular}{lrrrr}
    \toprule
    Method & \combvd & MacAdam & HumFB & Avg \\
    \midrule
    \textbf{MetricSpace v21} & \textbf{22.48} & \textbf{19.51} & \textbf{23.26} & \textbf{21.75} \\
    CIE94                    & 33.37 & 19.78 & 59.75 & 37.63 \\
    CIEDE2000                & 29.20 & 22.13 & 62.54 & 37.96 \\
    DIN99                    & 35.76 & 23.28 & 56.46 & 38.50 \\
    $J_z a_z b_z$ (Euclid.)  & 40.63 & 25.50 & 52.93 & 39.69 \\
    CIE Lab ($\Delta E^*_{76}$) & 42.86 & 24.53 & 62.32 & 43.24 \\
    Oklab ($\Delta E_\mathrm{OK}$) & 47.35 & 32.72 & 57.27 & 45.78 \\
    CAM16-UCS (Euclid.)      & 56.19 & 33.15 & 65.89 & 51.74 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{Why the comparison favours MetricSpace.}
...
color-difference prediction. CAM16-UCS's \stress\ on this set
(56.19) is higher than the value we historically reported for
v20b's evaluation; the difference is implementation-detail (we
have switched to a stricter D65 reference and Bradford
chromatic adaptation in the \combvd\ pre-processing for
this revision, which affects every space, not just ours).
...
\Cref{fig:crossval} visualises cross-dataset performance. v21
outperforms CIEDE2000 on \emph{both} held-out validation sets:
\begin{itemize}
  \item He~\etal\ 2022 (82~pairs, regularizer): MetricSpace 23.26
    vs CIEDE2000 62.54. The He set is mostly large differences,
    where CIEDE2000's S-curve is poorly calibrated.
  \item MacAdam~1974 (128~pairs, fully held-out): MetricSpace 19.51
    vs CIEDE2000 22.13. v21 leads CAM16-UCS (33.15) and every other
    tested space here as well.
\end{itemize}
This is a change from v20b, which had trailed CAM16-UCS on
MacAdam. The 3-dataset balanced loss (\Cref{eq:loss}) appears to
have widened the optimum into a region that generalises better
to small held-out sets without the v20b-specific blue-band
penalty.
...
separately. The \stress\ increase from the neutral correction is
small (sub-tenth in our v21 ablation): the underlying achromatic
curve is smooth, so PCHIP interpolation introduces negligible
error, and the optimiser had already converged on a basin where
$\bar{a}_\mathrm{err}, \bar{b}_\mathrm{err}$ are well below 0.1.
Achromatic max chroma drops from $\sim 0.34$ (measurement-only
baseline) to $< 10^{-6}$.
...
entirely ($C < 10^{-6}$); GenSpace (\Cref{sec:genspace}) achieves the
...
\begin{table}[t]
  ...
  \begin{tabular}{lr}
    ...
    Round-trip $\|\cdot\|_\infty$ (sRGB)   & $\sim$1$\times$10$^{-13}$ \\
    Round-trip $\|\cdot\|_\infty$ (P3)     & $\sim$2$\times$10$^{-13}$ \\
    Munsell CV & 20.0\% \\
    ...
    He \stress & 23.26 \\
    MacAdam \stress (fully held out) & 19.51 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{lrr}
    ...
    No H-K embedding ($w_\mathrm{HK}=0$) & $\sim$26.5 & $+4.0$ \\
    No hue correction (Stage 4)  & $\sim$37.9 & $+15.4$ \\
    No dark-L compression (Stage 6 part) & $\sim$22.7 & $+0.2$ \\
    \midrule
    Rotation $\varphi=-28.2$\textdegree    & 22.48 & 0.00 \\
    No rotation (set $\varphi=0$)         & 22.48 & 0.00 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrr}
    \toprule
    Sub-dataset & $N$ & MetricSpace v21 & CIEDE2000 \\
    \midrule
...
shape:
\[
\mathrm{XYZ} \to \mathbf{M}_1 \to \mathrm{depcubic}(\alpha\!=\!0.021) \to \mathbf{M}_2 \to C^{0.978} \to \mathrm{PW}_L^{19} \to \mathrm{enr}_h(L) \to \mathrm{Lab}
\]
...
\paragraph{Result.}
GenSpace v0.11.1 wins 66 of 90 metrics versus OKLab across a
...
\begin{table}[t]
  \centering
  \caption{GenSpace v0.11.1 vs OKLab — head-to-head on ColorBench (83 internal + 7 independent gradient/palette metrics).}
  \label{tab:genspace-vs-oklab}
  \begin{tabular}{lrrr}
    \toprule
    Category & GenSpace & OKLab & Ties \\
    \midrule
    Gamut geometry              & \textbf{25} & 0 &  2 \\
...
(19.51 vs 22.13) and on a Human-Feedback judgement set (23.26 vs
62.54). On a held-out 20\% test split, the gap is +1.77 \stress\
($\sim$24.3 cross-validated point estimate); even at this estimate,
\helmlab\ remains the best-performing space tested. The achromatic
axis is guaranteed to have $C < 10^{-6}$ via the post-pipeline
neutral correction, and the rigid rotation brings hue error down to
18.1\textdegree\ RMS without affecting the distance metric (the
metric is provably invariant under any rigid $(a,b)$ rotation).
...
it wins 66 vs OKLab. The two spaces together cover the
...
\bibitem{colorjs}
L.~Verou, C.~Lilley, et al., ``Color.js --- a library for color
conversions, manipulation, and difference computation,''
\url{https://colorjs.io/}, 2026. \helmlab\ integration: PR \#722,
\url{https://github.com/color-js/color.js/pull/722} (merged
2026-05-04).

\bibitem{helmlab-pkg}
G.~Y{\i}ld{\i}z, ``\texttt{helmlab} --- production color-space package
(Python on PyPI, JavaScript on npm),''
\url{https://pypi.org/project/helmlab/},
\url{https://www.npmjs.com/package/helmlab}, 2026.

\bibitem{postcss-helmlab}
G.~Y{\i}ld{\i}z, ``\texttt{postcss-helmlab} --- PostCSS plugin for
\helmlab\ CSS color functions,''
\url{https://www.npmjs.com/package/postcss-helmlab}, 2026.

\bibitem{ottosson-cusp}
B.~Ottosson, ``How software gets color wrong (and what we can do
about it): Oklab cusp behaviour and the cubic-fold artefact,''
discussion thread, \url{https://github.com/color-js/color.js/issues/81}
and CSS WG drafts, 2022--2024.
