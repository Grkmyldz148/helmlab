...
\begin{table}[t]
  ...
  \begin{tabular}{lr}
    ...
    Property & Value \\
    \midrule
    Achromatic max $C$ (NC on)            & $<10^{-6}$ \\
    Hue RMS (6 sRGB primaries)            & 26.4\textdegree \\
    Hue max error                         & 53.4\textdegree \\
    Round-trip $\|\cdot\|_\infty$ (sRGB)   & $\sim$7$\times$10$^{-15}$ \\
    Round-trip $\|\cdot\|_\infty$ (XYZ random) & $\sim$1$\times$10$^{-15}$ \\
...
$\varphi=-28.2$\textdegree\ for backwards compatibility with v20b
deployments. Applications that need accurate primary hue
specification should compose MetricSpace with a per-application
hue post-rotation, or use GenSpace (\Cref{sec:genspace}) where
the geometry is preserved by construction.
...
tested. The achromatic axis is guaranteed to have $C < 10^{-6}$
